...
def box2corners_th(box:torch.Tensor)-> torch.Tensor:
    ...
    a = corners.view([-1,4,2]).type(torch.float32)
    b = rot_T.view([-1,2,2]).type(torch.float32)
    rotated = torch.bmm(a, b)
...
def cal_ciou(box1:torch.Tensor, box2:torch.Tensor, enclosing_type:str="smallest"):
    ...
    Args:
        ...
        box2 (torch.Tensor): [description]
    """
    diou_loss, iou = cal_diou(box1, box2)
    w_gt, h_gt = box2[..., 2], box2[..., 3].type(torch.float32)
    w_pred, h_pred = box1[..., 2], box1[..., 3].type(torch.float32)
    v = (4/(math.pi*math.pi)) * (torch.pow(torch.atan(w_gt/h_gt) - torch.atan(w_pred/h_pred),2))
    alpha = v / ((1.-iou)+v + 1e-7)   # +eps guards the iou=1, v=0 degenerate (0/0 -> NaN)
