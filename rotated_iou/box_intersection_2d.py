...
import torch
try:
    # Real CUDA kernel (build on the GPU server: cd cuda_op && python setup.py install)
    from .cuda_op.cuda_ext import sort_v
except Exception:
    # Pure-PyTorch CPU fallback so the loss imports/runs without CUDA (see AOD note).
    from aod_yolov8._sort_cpu import sort_v
EPSILON = 1e-8
